...
class StepAudioTTS:
    ...
    def register_speakers(self):
        ...
        #fix at 25/7/27 remove '..' in l 126&130 cause it didn't actually exist
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),"speakers","speakers_info.json"), "r", encoding='utf-8') as f:#changed to abs path and encoding into utf-8
            speakers_info = json.load(f)

        for speaker_id, prompt_text in speakers_info.items():
            prompt_wav_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),"speakers",f"{speaker_id}_prompt.wav")
